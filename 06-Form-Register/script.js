...
form.addEventListener("submit",function(e){
    ...
    const isConfirmPasswordRequired = checkRequired([confirmPassword]);

    const isRequiredValid = isUsernameRequired && isEmailRequired && isPasswordRequired && isConfirmPasswordRequired;
...
function checkLength(input, min, max){
    if (input.value.length < min || input.value.length > max) {
        showError(input,`${formatFieldName(input)} must be between ${min} and ${max} characters`);
        return false;
    } else {
        showSuccess(input);
        return true;
